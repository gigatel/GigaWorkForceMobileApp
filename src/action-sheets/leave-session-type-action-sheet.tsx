...
const LeaveSessionTypeActionSheet = ({
  value,
  setValue,
}: {
  ...
  return (
    <>
      <DropdownInput
        title={'Leave Session Type'}
        disabled={false}
        value={value?.name ?? 'Select'}
        placeholder={'Select Leave Session Type'}
      ...
      <OptionPickerSheet
        ...
        type={'single'}
        title={'Select Leave Session Type'}
        data={DATA?.LEAVE_SESSION_TYPE ?? []}
        rowUniqueKey={'id'}
        enableSearch={false}
        // searchKeys={['name', 'id']}
        onClose={() => {
...
export default memo(LeaveSessionTypeActionSheet);
