...
import {createTicketFollowup} from '@slices/tickets.slice';
...
type StartTicketRouteProp = RouteProp<RootStackParamList, 'StartTicketScreen'>;

const DEFAULT_ASSIGN_TASK_ID = 8256;
const ADDR_ERR = 'Unable to fetch current location ...';

const StartTicketScreen: React.FC<Props> = ({
  onSubmit,
  onCancel,
  assignTaskId,
}) => {
  ...
  useEffect(() => {
    console.log('[StartTicketScreen] route params raw:', route?.params);
    console.log('[StartTicketScreen] ticket summary:', {
      LinkId: ticket?.LinkId,
      assignId: ticket?.assignId,
      cutLocation: ticket?.cutLocation,
    });
    console.log('[StartTicketScreen] route.assignTaskId:', routeAssignTaskId);
    console.log('[StartTicketScreen] prop.assignTaskId:', assignTaskId);
    console.log('[StartTicketScreen] from:', from);
  ...
  const effectiveAssignTaskId = useMemo(() => {
    ...
    console.log('[StartTicketScreen] effectiveAssignTaskId:', {
      fromTicket,
      fromRoute,
      fromProp,
      used: finalVal,
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>Start Ticket</Text>
        ...
        <View style={styles.footer}>
          <TouchableOpacity
            ...
            <Text style={styles.startBtnText}>
              {submitting ? 'SUBMITTING…' : 'START TICKET'}
            </Text>
          </TouchableOpacity>
        </View>
      </ScrollView>
...
export default StartTicketScreen;
/* -------------------- STYLES -------------------- */
